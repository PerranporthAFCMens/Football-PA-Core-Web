...
const nav=fs.readFileSync('core-nav.js','utf8');try{new Function(nav)}catch(e){fail('core-nav.js',`syntax error: ${e.message}`)}if(/deploy refresh/i.test(nav))fail('core-nav.js','temporary deployment marker still present');if(!nav.includes("get_team_access_context")||!nav.includes('access.can_manage_voting')||!nav.includes('access.can_manage_subs')||!nav.includes('access.can_manage_team'))fail('core-nav.js','capability-based navigation access is missing');if(!nav.includes("select('scoreboard_token,nav_order')")||!nav.includes('teamItems.sort'))fail('core-nav.js','saved team navigation order is not applied');if(!nav.includes("['voting','Voting'")||!nav.includes("'voting',canManage"))fail('core-nav.js','feature-gated Voting navigation missing');if(!nav.includes("['subs','Subs Tracker'")||!nav.includes("'subs_finance',canManage"))fail('core-nav.js','feature-gated Subs Tracker navigation missing');
